...
interface Props {
  onDone: (imageCID: string, pinataConfig: PinataConfig) => void;
}

export default function UploadImage({ onDone }: Props) {
  ...
  // Persist Pinata keys in sessionStorage
  useEffect(() => {
    const saved = sessionStorage.getItem("pinata_keys");
    if (saved) {
      try {
        const { apiKey: ak, secretKey: sk } = JSON.parse(saved);
        if (ak) setApiKey(ak);
  ...
  useEffect(() => {
    if (apiKey || secretKey) {
      sessionStorage.setItem("pinata_keys", JSON.stringify({ apiKey, secretKey }));
    }
  }, [apiKey, secretKey]);
  ...
  function useCID(hash: string, requireKeys = true) {
    if (requireKeys && (!apiKey || !secretKey)) {
      setError("Pinata API keys are required (for metadata upload later)");
      return;
    }
    onDone(hash, { apiKey, secretKey });
  }

  const tabs = [
    { key: "upload" as const, label: "Upload New" },
    { key: "existing" as const, label: "Use Existing CID" },
  ];
                ...
                <div key={pin.ipfs_pin_hash} className="flex items-center justify-between px-3 py-2.5 border-b border-[#e2e8f0] last:border-b-0">
                  ...
                  <div className="flex gap-1 shrink-0">
                    <button
                      ...
                      className="px-2 py-1 text-[10px] rounded bg-[#0ea5e9] hover:bg-[#0284c7] text-white transition-colors"
                    >
                      Use
                    </button>
                    <button
      ...
      {tab === "existing" && (
        <div className="space-y-4">
          <div className="flex gap-2">
            ...
            />
            <button
              onClick={() => { if (manualCID.trim()) useCID(manualCID.trim(), false); }}
